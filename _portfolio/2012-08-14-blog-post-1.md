...
<img src="./images/base_fl.gif" alt="Base Gif Frozen Lake" width="200px" style="float: left; box-shadow: 0 0 10px rgba(0, 0, 0, 0.3);">
...
Our agent begins its journey at the top-left corner of an 8x8 grid representing a lake and must reach its goal at the bottom-right corner. The challenge being the holes in the ice, where stepping on them will result in failure. Our agent then needs to learn which path is safe by interacting with the environment. 

The agent can move in four directions: left, down, right and up. But here's the catch: when the environment parameter `is_slippery=True`, the intended action will only take place 1/3 of the time. The other 2/3 of the time, the agent will slide perpendicular to the intended direction. 
...
4. It updates its knowledge within using the Q-learning update formula. It stores this new knowledge within a Q-table (64x4), grouping the calculated Q-values in each of the 64 states (the positions on the lake) that the agent could be in and 4 actions (up, down, left, right) that the agent could take. 
...
for i in range(episodes):
  ...
  while(not terminated and not truncates):
    ...
    if is_training:
      q[state, action] = q[state, action] + learning_rate_alpha * (reward + discount_factor_gamma * np.max(q[new_state])) - q[state, action]
...
After training the agent for about 15 000 episodes, we can observe the following results:

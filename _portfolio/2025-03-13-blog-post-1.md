...
The agent can move in four directions: left, down, right and up. But here's the catch: when the environment parameter `Try the app` , the intended action will only take place $$\frac{1}{3}$$ of the time. The other $$\frac{2}{3}$$ of the time, the agent will slide perpendicular to the intended direction. 
